...
def run(idProduct):
    ...
    init_angle,ts=Test_Surface()
    #Initialise uarm
    print('')
    init_z,arc=iniz(init_angle)
    ...
    if total == 0:
        ...
        while True:
            ...
            plt.show()
            output_dir='/Users/yf_choong/Desktop/Result/dpi_mm/'
            plt.savefig(output_dir + ts)
            if add_swing==0 and acc_swing==0:
                break
       
        display(init_z,count,idProduct)
        print("Close plot and restart?  y/n")
        inp=str(input())
        if inp != 'y' and inp != 'n' :
            print('Invalid Input')
            while True:
...
def iniz(init_angle):
    ...
    return (init_z,arc)
#Initialise test surface
def Test_Surface():
    print("Enter Test Surface:")
    ts=str(input())
    ...
    if tn>=4 or tn<0:
        ...
        while True:
            ...
            if tn>=4 or tn<0:
                ...
                break
    print("Test surface: ",ts)
    print("Mat thickness: ",tn)
    init_angle= 15.9 - (tn* 0.4)
    return(init_angle,ts)
#Display Result
def display(init_z,count,idProduct):
     h=sg()
