# -*- coding: utf-8 -*-
"""
Created on Fri Aug 23 11:20:02 2019
...
def main(): 
    ...
    init_angle=calibrate(idProduct)
    ...
    while loop_main<4:
        ...
        run(sf,angle_rot,sub_dir,idProduct,init_angle) 
        time.sleep(0.5)
        #Close all plottings
        plt.close(fig='all')    
        print('')
...
def create_dev():
    ...
    if dev is None:
        ...
        print('')
    #while True:
        #c_off=float(input("Enter approximate cut-off height: " ))
        #if c_off>0:
            #break
        #else:
...
def reset():
    ...
    #servo id 2 = height
    swift.set_servo_angle(servo_id=2,angle=10,speed=10000)
    return
def calibrate(idProduct):
    ...
    swift.flush_cmd(wait_stop=True)
    while True:
        c_off=float(input("Enter app. cut-off height (mm): " ))
        if c_off>0:
            break
        else:
    ...
    b=((c_off/0.65)/2)
    a=ss()
    swift.set_servo_angle(servo_id=2,angle=a[2]+b,speed=10000)
    swift.flush_cmd(wait_stop=True)
    j=ss()
    init_angle=j[2]
    swift.set_servo_angle(servo_id=2,angle=5,speed=10000)
...
def run(sf,angle_rot,sub_dir,idProduct,init_angle):
    #Initialise robotic arm position
    init_z,arc=initz(angle_rot,init_angle)
    ...
    if total == 0:
        ...
        while True:
            ...
            while True:
                if count1==0:
                    swift.set_servo_angle(servo_id=2,angle=init_angle-t2,speed=7500)
                    time.sleep(0.1)
                    while True:
                        h=sg()
                        H2=h[2]
...
def initz(angle_rot,init_angle):
