# -*- coding: utf-8 -*-
"""
Created on Mon Jul 29 08:33:42 2019
...
def run(idProduct):
    ...
    if total!=0:
        ...
        count=1
        while True:
            swift.set_servo_angle(servo_id=2,angle=15.8-t,speed=10000)
            time.sleep(0.7)
            f=ss()
            a_a.append(f[2])
            g=sg()
...
def reset():
    ...
    swift.set_servo_angle(servo_id=2,angle=15.8,speed=10000)
    swift.flush_cmd(wait_stop=True)
...
def swing(idProduct):
    ...
    x1=[]
    y1=[]
    swift.set_servo_angle(servo_id=0,angle=70,speed=10000)
    for l in range (0,15):
        e=sg()
        a_zs.append(e[2])
        f=ss()
    ...
    for l in range (0,15):
        ...
        a_dx.append(x2)
        a_dy.append(y2)
    swift.set_servo_angle(servo_id=0,angle=110,speed=10000)
    x3=[]
    y3=[]
    for l in range (0,5):
       e=sg()
